...
implementation {
  ...
  //increase or decrease transmit rate
  uint16_t succSendCnt;
  ...
  event void Boot.booted() {
	...
	//increase or decrease transmit rate
	succSendCnt = 0;
	transInterval = DEFAULT_INTERVAL;
  ...
  */
  ...
  event void Timer0.fired() {
	...
	if (cii >= ciiThreshold) {
		...
		succSendCnt++;
		if (succSendCnt == 5){
			if(transInterval >= 320){
				transInterval = transInterval - 56;
				call Timer0.startPeriodic(transInterval);
			}
		}
